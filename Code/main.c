...
// Channel 0..125
#define	_CH1 1  //robot 1's frequency		    
...
int main(void) {
	...
	while(1)
	{
		...
		for(i=0; i < 6; i++)
		{
			...
			LED_OFF;
			//Process response and move servo
			if(Buf[0] == '\0') continue;
			pitch = atoi(Buf);
			pitch += 150;
...
/******************************************************************************************************/
	...
	uint16_t i, j;
	uint8_t CH;
	uint8_t top_bot, right_left, robot1_2, ID = 0;
	...
	//Read switches to determine arm segment and robot
	...
	delay32Ms(1,2000);  //device setup delay   
	...
	while (1) 
	{
			...
			enable_timer32(0);	
		   	...
		   	//Print every few cycles for debug
			i++;
			if(i==0xF)
			{  
				printf("Kalman_X = %.4f\r\n", kal_angle);
			...
			if( LPC_GPIO1->MASKED_ACCESS[(1<<5)] == 0 )	//check if request has been made
			{
				NRF24L01_Receive(Buf);
				...
				{
					//if request for position, transmit current value
					int angle;
					LED_ON;
				 	NRF24L01_Set_Device_Mode(_TX_MODE);
					for(j=0; j < _Buffer_Size; j++) Buf[j] = '\0';
